...
class InProgressReportsScreen extends StatelessWidget {
  @override
    ...
    return BlocConsumer<SuperAdminGpCubit, SuperAdminGPState>(
      ...
      builder: (context, state) {
        return ConditionalBuilder(
          condition: SuperAdminGpCubit.get(context).inProgress != null,
          builder: (context) => Scaffold(
            backgroundColor: Color(0xff031B31),
            body: Padding(
              padding: const EdgeInsets.all(8.0),
              child: ConditionalBuilder(
                condition: SuperAdminGpCubit.get(context).refuese.length > 0,
                builder: (context) => ListView.separated(
                    itemBuilder: (context, index) => reportWidget(
                        SuperAdminGpCubit.get(context).inProgress[index]),
                    separatorBuilder: (context, index) => SizedBox(
                          height: 10,
                        ),
                    itemCount:
                        SuperAdminGpCubit.get(context).inProgress.length),
